...
The second problem we encounter is that of reading and writing nodal
...
\begin{algorithm}[p]
  \begin{minipage}{.5\textwidth}
    %\begin{subalgorithm}{\textwidth}
      ...
      \begin{algorithmic}[1]
        \Procedure{FElastic}{$\bm{x}$, params $p$}\hspace{.1cm} \tikzmark{right}
        \State \Reshape \chighlight{$\bm{x} \rightarrow \bm{D}_s$}
        \State \Compute \chighlight{$\bm{F} \leftarrow
          \bm{D}_s\bm{G}^T$}
        \State \Compute \chighlight{$[U,\Sigma,V] \leftarrow
          \Call{SVD}{\bm{F}}$}\tikzmark{top}
        \State \Compute $\hat{\bm{P}} \leftarrow
        \Call{P\_hat}{\Sigma, p}$
        \State \Compute $\bm{P} \leftarrow
        \bm{U\hat{P}V}^T$\tikzmark{bottom}
        \State \Set $\bm{H} \leftarrow
        V_0\bm{PG}$
        \State \Reshape $\bm{H} \rightarrow \bm{f}$
        \EndProcedure
      \end{algorithmic}
      \AddNote{top}{bottom}{right}{$\bm{P(F)}$}    
    %\end{subalgorithm}
  \end{minipage}
  ...
  \begin{minipage}{.5\textwidth}
    %\begin{subalgorithm}{\textwidth}
      \setlength{\baselineskip}{1.35em}
      \begin{algorithmic}[1]
        \Procedure{$\delta$FElastic}{$\bm{x}$,$\bm{\delta{}x}$, params $p$} 
        \State \Reshape \chighlight{$\bm{x} \rightarrow
          \bm{D}_s$}, $\bm{\delta{}x} \rightarrow \bm{\delta{}D}_s$
        \State \Compute \chighlight{$\bm{F} \leftarrow
          \bm{D}_s\bm{G}^T$}, $\bm{\delta{}F} \leftarrow
        \bm{\delta{}D}_s\bm{G}^T$\tikzmark{right}
        \State \Compute \chighlight{$[U,\Sigma,V] \leftarrow
          \Call{SVD}{\bm{F}}$}\tikzmark{top}
        \State \Compute \chighlight{$\mathcal{T} \leftarrow
          \Call{dPdF\_hat}{\Sigma, p}$}
...
\shortcite{McAdaZSETTS:2011} which replicates almost
